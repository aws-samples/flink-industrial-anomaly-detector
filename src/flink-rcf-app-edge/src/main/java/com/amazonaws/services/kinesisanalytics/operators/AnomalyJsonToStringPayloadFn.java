...
public class AnomalyJsonToStringPayloadFn extends RichMapFunction<String, String> {
	protected final ParameterTool parameter;
	private final Logger logger = LoggerFactory.getLogger(getClass());
	
	public AnomalyJsonToStringPayloadFn(ParameterTool parameter) {
	...
	private static enum measureTypes {
		CONTINUOUS, SAMPLED_REACTOR_FEED_STREAM6, SAMPLED_PURGE_GAS_ANALYSIS_STREAM9, SAMPLED_PRODUCT_ANALYSIS_STREAM11;
	}
	 ...
	 */
	@Override
	public String map(String jsonString) {
			...
			timestreamMeasures.put("proc_time_ms", String.valueOf(Instant.now().toEpochMilli()));
			
			ArrayList<String> group1 = tepMeasureVariables(measureTypes.SAMPLED_REACTOR_FEED_STREAM6);
			ArrayList<String> group2 = tepMeasureVariables(measureTypes.SAMPLED_PURGE_GAS_ANALYSIS_STREAM9);
			ArrayList<String> group3 = tepMeasureVariables(measureTypes.SAMPLED_PRODUCT_ANALYSIS_STREAM11);
			...
			//#######
			String response =  new Gson().toJson(timestreamMeasures);
